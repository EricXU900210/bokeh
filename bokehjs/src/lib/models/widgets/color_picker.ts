...
function isDescendant(parent: HTMLElement, child: Element | null) {
  if (child == null) {
    return false
  } else {
...
class GradientPanelView extends Signalable() {
  ...
  _update_cursor_position(pageX: number, pageY: number): void {
    const {
      left: pn_left,
      top: pn_top,
    } = this.gradient_panel_el.getBoundingClientRect()
    const pos_x = pageX - pn_left
    const pos_y = pageY - pn_top
    this._set_cursor_pos_x(pos_x)
    this._set_cursor_pos_y(pos_y)
    this.model.setv({
      saturation: this._pos_x_to_saturation(pos_x),
      lightness: this._pos_y_to_lightness(pos_y),
    })
  }

  _set_cursor_pos_x(pos_x: number): void {
    const {width: cursor_width} = this.cursorEl.getBoundingClientRect()
  ...
  _set_cursor_pos_y(pos_y: number): void {
    const {height: cursor_height} = this.cursorEl.getBoundingClientRect()
  ...
  _process_drag_stop(_ev: MouseEvent): void {
...
class HueSliderView extends Signalable()  {
  ...
  constructor(hue: number) {
    ...
    this.hue_slider_el.appendChild(this.cursorEl.left)
    this.hue_slider_el.appendChild(this.bar)
    this.hue_slider_el.appendChild(this.cursorEl.right)
  ...
  _set_cursor_pos(pos: number) {
    const {height: cursor_height} = this.cursorEl.left.getBoundingClientRect()
    const cursor_margin =
      Math.round(pos - cursor_height / 2 + 1).toString() + "px"
